var db               = require('../../models'),
  ...
  exports.loadRoutes = function(endpoint, apiRoutes) {
    ...
    apiRoutes.get(endpoint, function(req, res) {
      db.atendimentos.findAll().then(function(entities) {
        res.statusCode = 200;
    ...
    apiRoutes.get(endpoint + '/:id', function(req, res) {
      db.atendimentos.find({where: {id: req.param('id')} }).then(function(entity) {
        res.statusCode = 200;
    ...
    apiRoutes.post(endpoint, function(req, res) {
      db.atendimentos.create(req.body).then(function(entity) {
        res.statusCode = 200;
        res.json(entity)
      }).catch(function(errors) {
        return controllerHelper.writeErrors(res, errors);
